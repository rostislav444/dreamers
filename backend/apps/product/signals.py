...
from apps.product.models import (
    Camera,
    CameraInteriorLayer,
    CameraInteriorLayerMaterial,
    CameraInteriorLayerMaterialGroup,
    Product,
    Product3DBlenderModel,
    ProductClass,
...
def generate_sku_from_options(sender, instance, **kwargs):
    if instance.pk and instance.generate_sku_from_options:
        ...
        ProductClass.objects.filter(pk=instance.pk).update(
            generate_sku_from_options=False
        )
    ...
    if instance.pk:
        ...
        if instance.product_class.materials_set:
            for part in instance.product_class.materials_set.parts.all():
                obj, _ = ProductCustomizedPart.objects.get_or_create(
                    part=part, product=instance
                )

        if instance.generate_sku_from_materials:
            materials_set = instance.product_class.materials_set
            if materials_set:
                # Получаем все рекомендованные комбинации для данного набора материалов
                for combination in RecommendedCombinations.objects.filter(
                    material_set=materials_set
                ):
                    # Получаем все материалы из частей рекомендованной комбинации
                    materials = []
                    for combo_part in combination.parts.all():
                        if combo_part.material:
                            materials.append(combo_part.material)

                    if not materials:
                        continue

                    # Проверяем, существует ли уже SKU с таким набором материалов
                    existing_sku = None
                    for sku in instance.skus.all():
                        if set(sku.materials.all()) == set(materials):
                            existing_sku = sku
                            break

                    if existing_sku:
                        # Если SKU с таким набором материалов уже существует,
                        # проверяем нужно ли генерировать изображения
                        if not existing_sku.images.exists():
                            existing_sku.generate_images = True
                            existing_sku.save()
                        continue

                    # Создаем SKU с выключенной генерацией изображений
                    sku, created = Sku.objects.get_or_create(
                        product=instance,
                        code=str(combination),
                        defaults={
                            "generate_images": False
                        },  # Отключаем генерацию изображений при создании
                    )

                    # Устанавливаем материалы для SKU
                    if materials:
                        sku.materials.set(materials)
                        sku.save()

                    # Теперь можно включить генерацию изображений
                    if created or not sku.images.exists():
                        sku.generate_images = True
                        sku.save()

                # Сбрасываем флаг после обработки
                Product.objects.filter(pk=instance.pk).update(
                    generate_sku_from_materials=False
                )
...
def calculate_camera_radius(
    width_m, height_m, depth_m, fov_degrees, aspect_width=3, aspect_height=2
):
    ...
    fov_vertical_radians = 2 * math.atan(
        math.tan(fov_horizontal_radians / 2) * (aspect_height / aspect_width)
    )
...
def get_rotation_coordinates(product: Product, instance: Product3DBlenderModel):
    ...
    fov_degrees, steps, eye_level, render_from_eye_level = (
        instance.fov_degrees,
        instance.steps,
        instance.eye_level,
    ...
    rad_x, camera_level = get_camera_level(
        height_m, radius, eye_level, render_from_eye_level
    )
    ...
    for i in range(steps):
        ...
        result.append({"location": (x, y, camera_level), "angle": (rad_x, 0, rad_z)})
    return result
...
def create_scene_product_part_materials(camera_instance):
    ...
    def create_product_part_scene_materials(product_part_scene, part):
        for material_group in part.material_groups.all():
            for material in material_group.materials.all():
                ProductPartSceneMaterial.objects.get_or_create(
                    part=product_part_scene, material=material
                )

    def create_product_part_scenes(materials_set):
        for part in materials_set.parts.all():
            product_part_scene, _ = get_or_create_product_part_scene(
                camera_instance, part
            )
        ...
        if interior:
            for interior_layer in interior.layers.all():
                ...
                for material_group in interior_layer.material_groups.all():
                    camera_interior_layer_material_group, _ = (
                        CameraInteriorLayerMaterialGroup.objects.get_or_create(
                            layer=camera_interior_layer, material_group=material_group
                        )
                    )
                    for material in material_group.materials.all():
                        camera_interior_layer_material, _ = (
                            CameraInteriorLayerMaterial.objects.get_or_create(
                                group=camera_interior_layer_material_group,
                                material=material,
                            )
                        )
...
def merge_images(images):
    ...
    for img in opened_images[1:]:
        base_image.paste(
            img, (0, 0), img
        )  # Если изображения имеют прозрачность, передаем img как маску
    ...
    if instance.pk:
        if not instance.code or instance.code.strip() == "":
            ...
            for material in instance.materials.all():
                # Используем код материала или его имя
                material_group = str(material.group.id)
                material_code = str(material.id)
                material_codes.append('-'.join([material_group, material_code]))

            if material_codes:
                # Создаем код из названий материалов
                instance.code = "_".join(material_codes)
                instance.save(update_fields=["code"])

        if instance.generate_images:
            for model_3d in instance.product.model_3d.all():
                for camera in model_3d.cameras.all():
                    images = []
                    for camera_part in camera.parts.all():
                        sku_material = instance.materials.get(
                            group__product_part=camera_part.part
                        )
                        try:
                    ...
                    sku_image, _ = SkuImages.objects.get_or_create(
                        sku=instance, camera=camera
                    )
                    sku_image.image.save(
                        f"sku_{instance.pk}_camera_{camera.pk}.jpeg",
