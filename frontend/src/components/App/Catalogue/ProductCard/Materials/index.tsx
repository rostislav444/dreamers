...
const getSkusMaterials = (
  ...
  return skus.reduce((result, sku) => {
    ...
    const hasSelectedMaterials = selectedMaterialKeys.every(
      (key) => sku.materials[key] === selectedMaterials[key],
    );
...
const filterMaterials = ({
  ...
  skus,
  selectedMaterials = {},
}: FilterMaterialsProps): filterMaterialsResponse => {
  const skuMaterials = getSkusMaterials(skus, selectedMaterials);

  const newFilteredParts = parts.map((part) => {
    const filteredMaterials = part.materials.filter((material) =>
      skuMaterials[part.id].includes(material.id),
    );
...
export const ProductCardMaterial = ({
  product,
  setSelectedSku,
}: ProductCardMaterialProps) => {
          ...
          >
            {part.materials.map((material) => (
              <Flex
                ...
                borderWidth={2}
                borderColor={
                  selectedMaterials[part.id] === material.id
                    ? "brown"
                    : "transparent"
                }
              >
                {material.material?.color?.hex && (
                  <Box
                    w={8}
                    h={8}
                    backgroundColor={material.material.color.hex}
                  />
                )}
