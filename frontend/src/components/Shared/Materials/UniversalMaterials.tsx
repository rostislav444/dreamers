...
const MaterialItem = ({material, isSelected, isCatalog, onClick}: MaterialItemProps) => (
    <Box>
        <Box
            position='relative'
...
export const UniversalMaterials = ({
    ...
    return (
        <Box>
            {!isCatalog && <Text fontSize="xl" fontWeight="bold">Колір</Text>}
            {validParts.map((part, i) => (
                <Box key={part.id}>
                    <Flex
                        justifyContent='space-between'
                        alignItems='center'
                        onClick={() => !isCatalog && setShowAll(old => {
                                ...
                                >
                                    {getFlatMaterials(part).map(material => (
                                        <MaterialItem
                                            key={material.id}
                                            material={material}
                                            isSelected={selectedMaterials[part.blender_name]?.material === material.id}
                                            isCatalog={isCatalog}
                                            onClick={() => handleMaterialsSet(part, material.groupName, material)}
                                        />
                                    ))}
                                </Grid>
                            ) : (
